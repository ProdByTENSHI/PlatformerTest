...
#pragma endregion
  ...
  g_Window = glfwCreateWindow(g_WindowWidth, g_WindowHeight, g_WindowTitle,
                              nullptr, nullptr);
  ...
  if (_glInit != GLEW_OK) {
    std::cerr << "Could not initialize GLEW. Error: "
              << glewGetErrorString(_glInit);
    return;
  }
  ...
  glfwSetWindowSizeCallback(g_Window,
  ...
  // -- Testing
  {
    ...
    SpriteComponent *sprite =
        new SpriteComponent(g_ResourceManager->GetTexture("Wood.png"));
  ...
  }
  {
    ...
    TransformComponent *transform = new TransformComponent(
        glm::vec2(2.0f, 1.0f), glm::vec2(0.0f), glm::vec2(1.0f));
    SpriteSheetComponent *spriteSheet =
        new SpriteSheetComponent(g_ResourceManager->GetSpriteSheet("Player"));
...
void Game::Update() {
  ...
  // *g_Ecs->GetComponent<SpriteSheetComponent>(_spriteSheetEntity);
  ...
  while (!glfwWindowShouldClose(g_Window) && m_InitStatus) {
    ...
    _ss << g_WindowTitle << " Delta Time: " << g_DeltaTime
        << " | Camera Position " << g_Camera->m_Position.x << " "
