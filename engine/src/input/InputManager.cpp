...
namespace tenshi
{
	...
	InputProvider* InputManager::GetInputProvider(const std::string& name)
	{
		if (!m_InputProvider[name])
		{
			std::cout << "[INPUTMANAGER] No Input Provider with the Name " << name << " found!" << std::endl;
			return nullptr;
		}

		return m_InputProvider[name];
	}

	void InputManager::AddInputProvider(const std::string& name, InputProvider* provider)
	{
		m_InputProvider[name] = provider;
	}
}