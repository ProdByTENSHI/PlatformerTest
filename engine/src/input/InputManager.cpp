		...
		glfwSetKeyCallback(g_Window, [](GLFWwindow* window, i32 key, i32 scanCode, i32 action, i32 mods)
			{
				...
				{
				case GLFW_PRESS:
					g_InputManager->m_CurrentlyPressedKeys.push_back(key);
					g_InputManager->m_KeyHoldData.insert(std::make_pair(key, 0.0f));
					g_InputManager->OnKeyDown.Dispatch(key);
					break;

				case GLFW_RELEASE:
					g_InputManager->m_CurrentlyPressedKeys
						.erase(STLUtil::GetItOfElementInVec(g_InputManager->m_CurrentlyPressedKeys, key));
