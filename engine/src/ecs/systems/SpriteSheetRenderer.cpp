		...
		EventHandler<Entity, Signature> _onSignatureChange([this](Entity entity, Signature signature)
			{
				if (m_Signature == signature)
				{
					...
					SpriteSheetComponent& _spriteSheet = *g_Ecs->GetComponent<SpriteSheetComponent>(entity);
					...
					for (i32 i = 0; i < m_Batches.size(); i++)
					{
						...
						_batch->m_FrameToEntityId[_spriteSheet.m_CurrentFrame].push_back(entity);

						// -- Subscribe to OnFrameChange Event
						EventHandler<u32, u32> _onFrameChange([&_spriteSheet, entity, _batch](u32 previous, u32 index)
							{
					...
					SpriteSheetBatch* batch = new SpriteSheetBatch(_spriteSheet.m_SpriteSheet->m_Texture);
					...
					*/
					...
					const u32 _FRAME_COUNT = _spriteSheet.m_SpriteSheet->H_FRAMES * _spriteSheet.m_SpriteSheet->V_FRAMES;
					glNamedBufferStorage(batch->m_Vbo, _spriteSheet.m_SpriteSheet->FRAME_COUNT * 6 * sizeof(Vertex), nullptr,
					...
					// -- Subscribe to OnFrameChange Event
					EventHandler<u32, u32> _onFrameChange([&_spriteSheet, entity, batch](u32 previous, u32 index)
						{
	...
	void SpriteSheetRenderer::Render(Shader& shader)
	{
		...
		for (auto& batch : m_Batches)
		{
			...
			for (i32 i = 0; i < batch->m_FrameToEntityId.size(); i++)
			{
				...
				{
					g_MasterRenderer->m_EntityIdsUbo.SubBufferData((_baseOffset + j) * sizeof(i32), sizeof(i32),
